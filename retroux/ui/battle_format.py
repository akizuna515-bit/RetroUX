...
def format_battle_review_tooltip(review, in_battle: bool,
    ...
    ## `summary_rows` について（★いまは使わない）

    2026-08-11 の「戦況の行が切れて続きが見えない」を受けて、いったん先頭に
    画面の4行を入れた。⚠ 2026-08-19 に依頼者から「画面と被る」とあり**出さなく
    なった**（RX-0068）。引数は呼び出し側の互換のため残してある。
    代わりに先頭へ「見かた」（数字の意味）を置く。
    """
