...
class PartyPanel(QWidget):
    ...
    ★名前は RAM（`$0113`）から文字コード表で読む（2026-07-29 に解読。
      `ViewModel.party_names`）。⚠ 2026-08-21 訂正（RX-0010）: 以前ここに
      「RAM から読めていない（文字コード表が未確定）」とあったのは古い。
      `user_config.yaml` の `names` は上書き。読めない文字は推測で当てない。
...
class AiPanel(QWidget):
    ...
    ★スコアと次点との差は `actor_roles.lua` が出す（ツールチップ側で見せる）。
      ⚠「AI は heal_only・スコアはまだ無い」は古い（2026-08-21 訂正 / RX-0010）。このパネルには出さない。
