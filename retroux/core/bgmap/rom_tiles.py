...
- ~~`map_id` < `$2B` のうち、街以外（城など）の土台~~ ★2026-08-03 に解決: 種別がそのまま
  CHR 索引と分かり、街・城とも `order_for_map()` は `[1]`（城 `$07` 実測 / RX-0010 で訂正）
...
#:   ⚠ 2026-08-03 以降この表は**使っていない**（種別＝CHR 索引に置き換え）。★記録として残すだけ（2026-08-21 訂正 / RX-0010）。
BORDER_TO_ORDER = {
    0x01: [1],                 # 街
}
