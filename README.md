...
- 本ソフトはゲームの内容物を含みません。解析情報は相互運用のための最小限
  （RAM/ROM の番地と構造）で、敵のデータ表などの**値は起動時にお手元の ROM から読みます**。
...
ウィンドウ整列まで自動で行います。
★初回起動時は、モンスターの絵（図鑑用）と マップの大きさ表（地図用）を
ROM から自動生成します（`work\monster-art-rom\` / `work\map-data\maps.json`。
2回目以降は生成済みなので何もしません）。
...
| `work\map-data\maps.json` | **マップの大きさ表**（初回起動時に ROM から自動生成。地図の縮尺・種別に使う） |
